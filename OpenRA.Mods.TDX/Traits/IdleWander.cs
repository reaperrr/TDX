...
namespace OpenRA.Mods.TDX.Traits
{
	...
	public class IdleWanderInfo : ITraitInfo, Requires<MobileInfo>
	{
		...
		public object Create(ActorInitializer init) { return new IdleWander(init.Self, this); }
	}

	public class IdleWander : INotifyIdle, INotifyBecomingIdle
	{
		readonly Actor self;
		readonly IdleWanderInfo info;
		...
		public IdleWander(Actor self, IdleWanderInfo info)
		{
			this.self = self;
			this.info = info;
		...
		{
			mobile.ResolveOrder(self, new Order("Move", self, false) { TargetLocation = targetCell });
		}
	}
}
