...
{
	public class MissileXInfo : IProjectileInfo
	{
		...
		public IEffect Create(ProjectileArgs args) { return new MissileX(this, args); }
	}

	// TODO: double check square roots!!!
	public class MissileX : IEffect, ISync
	{
		enum States
		{
			Freefall,
		...
		public MissileX(MissileXInfo info, ProjectileArgs args)
		{
			this.info = info;
			this.args = args;
		...
		WVec HomingTick(World world, WVec tarDistVec, int relTarHorDist)
		{
			...
			var delta = OpenRA.Mods.Common.Util.NormalizeFacing(hFacing - desiredHFacing);
			...
			hFacing = OpenRA.Mods.Common.Util.TickFacing(hFacing, desiredHFacing, info.HorizontalRateOfTurn);
			vFacing = OpenRA.Mods.Common.Util.TickFacing(vFacing, desiredVFacing, info.VerticalRateOfTurn);
